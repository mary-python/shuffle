...
d = 1000; k = 10; n = 240000; t = 1; eps = 0.1; dta = 0.75; R = 3; V = 6
...
print(f"established that the choice t = {t} produces the optimal error for the algorithm. \n")
...
# This loop is completed over all V different values of m.
    ...
    loopTime = time.perf_counter(); m = (value + 1)*(50)
    # At the start of this "for loop", the current time is recorded, and will be subtracted from the time recorded at the
    # end of the loop. For the first loop, the value of m is set to 50, and is increased by 50 for every loop thereafter.
    ...
    # This loop is completed over all R repeats of the V different values of m.
        ...
        # This loop is completed over all n users in the R repeats of the V different values of m.
            ...
            randomVector = list(); sampledCoords = list(); submittedCoords = list()
            descaledCoords = list(); debiasedCoords = list()
            # These lists will be used to store the inputted vector from each user into OSS, as well as each vector resulting 
            # from each of the subsequent steps of sampling, submitting, descaling and debiasing.

            dftSampledCoords = list(); dftSubmittedCoords = list()
            dftDescaledCoords = list(); dftDebiasedCoords = list()
            # In the FSA, each inputted vector is transformed to the Fourier domain, after which OSS begins. The above lists will 
            # be used to store the vector resulting from each of the subsequent steps of sampling, submitting, descaling and debiasing.

            for i in range(0, d):
                randomVector.append(-(math.log(1 - (1 - math.exp(-5))*(random.SystemRandom().random())))/5)
            ...
            sampledPairs = random.sample(list(enumerate(randomVector)), t)
            # A small number t coordinates are uniformly sampled from each vector, with the index and value of each coordinate
            # recorded in an enumerated list.

            for idx, smp in list(enumerate(sampledPairs)):
                sampledCoords.append(smp[1])
            ...
            roundedPairs = [(idx, (math.floor(smp*k) + np.random.binomial(1, smp*k - math.floor(smp*k)))) for idx, smp in sampledPairs]
            ...
            if b == 0:
                submittedPairs = roundedPairs
            else:
                submittedPairs = [(idx, (np.random.randint(0, k+1))) for idx, rnd in roundedPairs]
            ...
            for idx, sbm in submittedPairs:
                submittedTotal[idx] += sbm
                submittedCoords.append(sbm)
                indexTracker[idx] += 1
            ...
            descaledCoords = [idx/k for idx in submittedCoords]
            debiasedCoords = [((5/6)*(abs((z - gamma/2)/(1 - gamma)))) for z in descaledCoords]
            ...
            for z in debiasedCoords:
                debiasedList.append(z)
            # This list aggregates all descaled and debiased coordinates from all users to be plotted on a histogram.

            for a, b in zip(debiasedCoords, sampledCoords):
                meanSquaredError += (a - b)**2
            ...
            dftSampledPairs = random.sample(list(enumerate(slicedDftVector)), t)
            ...
            for idx, smp in list(enumerate(dftSampledPairs)):
                dftSampledCoords.append(smp[1])
                dftSampledList.append(smp[1])
            # The value of each sampled coordinate is stored in two different lists. The former will be used in the calculation of 
            # the MSE for each particular user, before being reset for the next user. The latter will not be reset until the sampled
            # coordinates from all users are aggregated and plotted on a histogram.

            dftRoundedPairs = [(idx, (math.floor(smp*k) + np.random.binomial(1, smp*k - math.floor(smp*k))))\
                for idx, smp in dftSampledPairs]
            # Each sampled coordinate is scaled by a factor of k, then it is rounded to one of the two nearest integers. The closer
            # the coordinate is to an integer, the more likely it will be rounded to that integer.
            ...
            if b == 0:
                dftSubmittedPairs = dftRoundedPairs
            else:
                dftSubmittedPairs = [(idx, (np.random.randint(0, k+1))) for idx, rnd in dftRoundedPairs]
            # This "if-else" statement represents a randomised response mechanism, which is applied to each rounded coordinate.
            # This mechanism uses the above Bernoulli distribution to determine whether the rounded coordinate is changed
            # before it is submitted. If b == 0, the rounded coordinate is submitted, but if b == 1, the rounded coordinate
            # is changed to a random integer picked uniformly between 0 and k (the original domain scaled by a factor of k).

            for idx, sbm in dftSubmittedPairs:
                dftSubmittedTotal[idx] += sbm
                dftSubmittedCoords.append(sbm)
            ...
            dftDescaledCoords = [idx/k for idx in dftSubmittedCoords]
            dftDebiasedCoords = [((5/6)*(abs((z - gamma/2)/(1 - gamma)))) for z in dftDescaledCoords]
            # The scaling and bias applied to each sampled coordinate in the rounding step is removed. Some coordinates end up
            # just outside the original domain, so the result is scaled by a factor of 5/6 to fix this issue. The result will be 
            # used in the calculation of the MSE for each particular user, before being reset for the next user. It is much more
            # efficient to run the descaling and debiasing steps both within this loop and after the submitted coordinates from all
            ...
            for z in dftDebiasedCoords:
                dftDebiasedList.append(z)
            # This list aggregates all descaled and debiased coordinates from all users to be plotted on a histogram.

            for a, b in zip(dftDebiasedCoords, dftSampledCoords):
                dftMeanSquaredError += (a - b)**2
            # The MSE variable above aggregates the MSE calculated for all users, and is reset only after a repeat has been completed.
        ...
        finalTotal = abs(irfft(paddedTotal)).tolist()
    ...
    comparison = (d*(n**(1/3))*(t**(5/6)))*(((np.log(t/dta))**(2/3))/(eps**(4/3)))
    datafile.write(f"Theoretical Upper Bound for MSE: {round(float(comparison)/1000)*1000} \n")
    ...
    dftComparison = (m*(n**(1/3))*(t**(5/6)))*(((np.log(t/dta))**(2/3))/(eps**(4/3)))
    datafile.write(f"Theoretical upper bound for perturbation error: {round(float(dftComparison)/1000)*1000} \n")
